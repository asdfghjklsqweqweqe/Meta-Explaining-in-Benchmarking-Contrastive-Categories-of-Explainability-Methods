# -*- coding: utf-8 -*-
"""one_model_one_explainer_bottomup.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1NJmQUcX9oFPWO0p9gVoo7L5swj25LRQt
"""
...
class one_model_one_explainer:
    ...
    def prediction_score_after_adapting_to_global(self, a_trained_GNN_Model, test_dataset, explanations, num_classes):
        ...
        for cls_id in range(num_classes):
            for i, input_graph in enumerate(test_dataset):
                expl = explanations[cls_id][i]
                copy_input_graph = deepcopy(input_graph)
                graph_edges = [(source_index, target_index) for source_index, target_index in
                               copy_input_graph.edge_index.T.tolist()]
                for saliency, (source_index, target_index) in zip(expl, graph_edges):
                    if saliency != 1:
                        copy_input_graph.x[source_index] = torch.zeros_like(copy_input_graph.x[source_index])
                        copy_input_graph.x[target_index] = torch.zeros_like(copy_input_graph.x[target_index])
                copy_input_graph = copy_input_graph.to(self.device)
                if a_trained_GNN_Model.__class__.__name__ == "GCN_plus_GAP_Model":
                    Output_of_Hidden_Layers, pooling_layer_output, ffn_output, prediction = a_trained_GNN_Model(copy_input_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "DGCNN_Model":
                    final_GNN_layer_output, sortpooled_embedings, output_conv1d_1, maxpooled_output_conv1d_1, output_conv1d_2, to_dense, output_h1, dropout_output_h1, output_h2, prediction = a_trained_GNN_Model(copy_input_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "DIFFPOOL_Model":
                    concatination_list_of_poolings, prediction_output_not_softed, prediction = a_trained_GNN_Model(copy_input_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "GIN_Model":
                    mlps_output_embeds, mlp_outputs_globalSUMpooled, lin1_output, lin1_output_dropouted, lin2_output, prediction = a_trained_GNN_Model(copy_input_graph, None)
                else:
                    raise Exception("We cover GCN_plus_GAP, DGCNN, DIFFPOOL, and GIN.")
                predictions[cls_id].append(torch.squeeze(prediction).tolist()[cls_id])
        return predictions

    def __call__(self, a_trained_GNN_Model, explainer_name, test_dataset, num_classes, explainer_epoch,
                 explainer_learning_rate, generalization_threshold, test_dataset_batched):
        a_trained_GNN_Model = a_trained_GNN_Model.to(self.device)
        print("****           BottomUP Approach           ****")
        ...
        winner_label_prediction_scores_for_generalized_explanations = {}
        ...
        if explainer_name == "GNNExplainer":
            ...
            gnnexplainer_timing = perf_counter() - t1
            average_explanation_time = gnnexplainer_timing / (explainer_epoch * len(test_dataset))
            ...
            py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/BottomUp Approach on Graph Classification/Script/'
            sys.path.insert(0, py_path)
            import bottomup_approach_for_local_methods as bottomup_isomorphism_scoring
            ...
            for i in range(num_classes):
                ...
                winner_label_prediction_scores_for_generalized_explanations[i] = []
            for key, class_explanations in explanations.items():
                common_edges_finder = bottomup_isomorphism_scoring.profile_frequencies_by_edge_based_explanations(
                    list_of_saliencies=class_explanations, test_dataset=test_dataset, style="Node")
                for expl, graph in zip(class_explanations, test_dataset):
                    profile_one_hot = common_edges_finder(an_explanation=expl, a_graph=graph,
                                                          threshold=generalization_threshold)
            ...
            gnnexplainer_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in gnnexplainer_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    gnnexplainer_predictions_after_adaptation[int(test_graph.y)][graph_indx])

            for key, value in winner_label_prediction_scores_for_generalized_explanations.items():
                label_based_avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            print("avg_prediction_scores_for_generalized_explanations: ",
                  avg_prediction_scores_for_generalized_explanations)
            print("label_based_avg_prediction_scores_for_generalized_explanations: ",
                  label_based_avg_prediction_scores_for_generalized_explanations)
            ...
            # fid_plus_score = fid_plus_xmethod_example.my_fidelity(
            #     saliencies_for_multiple_classes=generalized_explanations_tensor,
            ...
            #     your_dataset=test_dataset, saliencies_for_multiple_classes=generalized_explanations_list,
            ...
            #     common_edges_finder = bottomup_isomorphism_scoring.profile_frequencies_by_edge_based_explanations(
            #         list_of_saliencies=class_explanations, test_dataset=perturbed_test_dataset, style="Node")
            ...
            #         generalized_explanations_for_perturbed_data_tensor[key].append(
            #             torch.from_numpy(np.array(profile_one_hot)))
            #
            # stability_score = stability.my_stability(
            #     normal_saliencies_for_multiple_classes=generalized_explanations_list,
            ...
            for key, class_explanations in explanations_tensor.items():
                common_edges_finder = bottomup_isomorphism_scoring.profile_frequencies_by_edge_based_explanations(
                    list_of_saliencies=class_explanations,
            ...
            pgexplainer_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in pgexplainer_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    pgexplainer_predictions_after_adaptation[int(test_graph.y)][graph_indx])
            ...
            #
            #     ExTrain_or_ExTest = 'train'
            #     EXP = PGExplainer_Module.PGExplainer(GNN_Model=a_trained_GNN_Model,
            #                                          explainer_save_index=explainer_epoch,
            ...
            #
            # for key, class_explanations in explanations_for_perturbed_data_list.items():
            #     common_edges_finder = bottomup_isomorphism_scoring.profile_frequencies(
            #         list_of_saliencies=class_explanations,
            ...
            import sys
            py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/GraphMask on Graph Classification/Script/'
            sys.path.insert(0, py_path)
            ...
            graphmask_timing = perf_counter() - pgex_t1
            average_explanation_time = graphmask_timing / (explainer_epoch * len(test_dataset))
            print(a_trained_GNN_Model.__class__.__name__, " Model by ", explainer_name, " average_explanation_time: ",
            ...
            graphmask_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in graphmask_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    graphmask_predictions_after_adaptation[int(test_graph.y)][graph_indx])
            ...
            #                                      Exp_Epoch=explainer_epoch, Exp_lr=0.001,
            #                                      explainer_hid_dim=test_dataset[0].x[0].size()[0],
            #                                      node_feat_dim=test_dataset[0].x[0].size()[0])
            #
            #     EXP(ExTrain_or_ExTest=ExTrain_or_ExTest, Exp_Load_index=explainer_epoch, your_dataset=test_dataset,
            #         target_class=class_index)
            ...
            import sys
            py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/SubGraphX on Graph Classification/Script/'
            sys.path.insert(0, py_path)
            ...
            subgraphx_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in subgraphx_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    subgraphx_predictions_after_adaptation[int(test_graph.y)][graph_indx])
            ...
            #     SubGX = SubGraphX_Module.SubGraphX(GNN_Model=GNN_Model, num_classes=num_classes, num_hops=2,
            #                                        explain_graph=True,
            #                                        rollout_count=20, min_children_threshold=5, ubc1_c_coef=10.0,
            #                                        expand_count_threshold=5, high2low=True, sample_num=100,
            #                                        save_permission_explanations=True,
            ...
            #     subgx_out_the_fly = SubGraphX_Module.SubGraphX_off_the_fly(test_dataset=perturbed_test_dataset,
            #                                                                Task_name='Graph Classification',
            #                                                                num_classes=num_classes,
            #                                                                dataset_name="Perturbed_" + str(
            #                                                                    self.dataset_name),
            ...
            #             explanations_for_perturbed_data_list[cls].append(
            #                 torch.sum(whole_data[i][cls]['important_nodes'].x, dim=1).tolist())
            #
            # # Adaptations for perturbed data
            # py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/BottomUp Approach on Graph Classification/Script/'
            # sys.path.insert(0, py_path)
            ...
            import sys
            py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/CF2 on Graph Classification/Script/'
            sys.path.insert(0, py_path)
            ...
            cf2_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in cf2_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    cf2_predictions_after_adaptation[int(test_graph.y)][graph_indx])
            ...
            for i in tqdm(range(num_classes)):
                for j in range(len(test_dataset)):
                    ...
                    pgm_node, p_values, candidate_nodes, dependent_nodes = pgmx.explain(
                        num_samples=len(test_dataset[j].x),
            ...
            pgmexplainer_predictions_after_adaptation = self.prediction_score_after_adapting_to_global(
                a_trained_GNN_Model, test_dataset, generalized_explanations_list, num_classes)

            avg_prediction_scores_for_generalized_explanations = {}
            label_based_avg_prediction_scores_for_generalized_explanations = {}

            for key, value in pgmexplainer_predictions_after_adaptation.items():
                avg_prediction_scores_for_generalized_explanations[key] = mean(value)

            for graph_indx, test_graph in enumerate(test_dataset):
                winner_label_prediction_scores_for_generalized_explanations[int(test_graph.y)].append(
                    pgmexplainer_predictions_after_adaptation[int(test_graph.y)][graph_indx])
            ...
            #                                                        perturb_indicator="abs")
            #         pgm_node, p_values, candidate_nodes, dependent_nodes = pgmx.explain(
            #             num_samples=len(perturbed_test_dataset[j].x),
            #             noise_offset_percentage=50, top_node=5,
            #             p_value_threshold=0.05, class_index=i)
            ...
            import sys
            py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/XGNN on Graph Classification/Script/'
            sys.path.insert(0, py_path)
            ...
            for cls_index in range(num_classes):
                ...
                if a_trained_GNN_Model.__class__.__name__ == "GCN_plus_GAP_Model":
                    Output_of_Hidden_Layers, pooling_layer_output, ffn_output, prediction = a_trained_GNN_Model(
                        explanation_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "DGCNN_Model":
                    final_GNN_layer_output, sortpooled_embedings, output_conv1d_1, maxpooled_output_conv1d_1, output_conv1d_2, to_dense, output_h1, dropout_output_h1, output_h2, prediction = a_trained_GNN_Model(
                        explanation_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "DIFFPOOL_Model":
                    concatination_list_of_poolings, prediction_output_not_softed, prediction = a_trained_GNN_Model(
                        explanation_graph, None)
                elif a_trained_GNN_Model.__class__.__name__ == "GIN_Model":
                    mlps_output_embeds, mlp_outputs_globalSUMpooled, lin1_output, lin1_output_dropouted, lin2_output, prediction = a_trained_GNN_Model(
                        explanation_graph, None)
                else:
                    raise Exception("We cover GCN_plus_GAP, DGCNN, DIFFPOOL, and GIN.")
            ...
            #     py_path = '/data/cs.aau.dk/ey33jw/Explainability_Methods/TopDown Approach on Graph Classification/Script/'
            #     sys.path.insert(0,py_path)
            ...
            explanations_size = {
                ...
                "Graph-SST5": {"GCN_plus_GAP_Model": {0: 10, 1: 9, 2: 9, 3: 9, 4: 20},
                               "DGCNN_Model": {0: 20, 1: 9, 2: 9, 3: 9, 4: 20},
            ...
            for i in range(num_classes):
                mean_of_node_embeddings[i] = torch.mean(
                    torch.cat([graph.x for graph in test_dataset if graph.y == i], dim=0), axis=0)
...
just_call_me(a_trained_GNN_Model=GNN_Model_Loaded, explainer_name=Explainer_Name, test_dataset=test_dataset,
             ...
             generalization_threshold=0.5, test_dataset_batched=test_dataloader)
